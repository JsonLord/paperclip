...
export function agentRoutes(db: Db) {
  ...
  const DEV_TEAM_ROLES = new Set(["cto", "engineer", "devops"]);

  function parseOwnerRepo(githubRepoUrl: string): string {
    const match = githubRepoUrl.match(/github\.com[/:]([^/]+\/[^/]+?)(?:\.git)?\/?$/);
    return match ? match[1]! : githubRepoUrl;
  }

  // Dashboard section appended to every agent's prompt so each agent can
  // report its own progress onto the company's shared Agent Dashboards page.
  // Isolation between agents is a prompt-level convention only — each agent
  // updates the data keys for its own area and leaves others intact.
  function buildDashboardSection(agentLabel: string, dashboardId: string | null): string {
    const dashUrl = process.env.DASHBOARDS_URL;
    const dashKey = process.env.DASHBOARDS_AGENT_KEY;
    if (!dashUrl || !dashKey || !dashboardId) return "";
    return (
      "\n\n## Company dashboard\n\n" +
      `Your company's dashboard (public view): ${dashUrl}/d/${dashboardId}\n` +
      `Update it via the Agent Dashboards API (header \`Authorization: Bearer ${dashKey}\`):\n` +
      `- Refresh values: \`PUT ${dashUrl}/api/dashboards/${dashboardId}/data\` with JSON \`{ "<dataKey>": <value> }\`.\n` +
      `- Adjust layout: \`PUT ${dashUrl}/api/dashboards/${dashboardId}\` with \`{ "widgets": [...] }\`.\n\n` +
      `You are "${agentLabel}". Report your own progress there as you work. ` +
      "Update the data keys for your own area and keep other agents' data intact."
    );
  }

  // Resolves the default adapterType/adapterConfig when the caller didn't
  // explicitly set one. Dev-team roles (cto/engineer/devops) route through
  // the hermes_local adapter, driving Jules via the Hermes Gateway
  // coordinator against the company's registered GitHub repo. Every other
  // role keeps the existing OpenOperator (http) default. Every role gets
  // the company's Homarr dashboard section appended to its prompt.
  async function resolveDefaultAdapter(
    companyId: string,
    role: string,
    agentName: string,
    explicitAdapterType: string | undefined,
    explicitAdapterConfig: Record<string, unknown> | undefined,
  ): Promise<{ adapterType: string; adapterConfig: Record<string, unknown> }> {
    if (explicitAdapterType) {
      return { adapterType: explicitAdapterType, adapterConfig: explicitAdapterConfig ?? {} };
    }

    const company = await db
      .select({
        name: companies.name,
        githubRepoUrl: companies.githubRepoUrl,
        julesAgentSlot: companies.julesAgentSlot,
        homarrBoardSlug: companies.homarrBoardSlug,
      })
      .from(companies)
      .where(eq(companies.id, companyId))
      .then((rows) => rows[0] ?? null);
    const dashboardSection = buildDashboardSection(agentName, company?.homarrBoardSlug ?? null);

    if (DEV_TEAM_ROLES.has(role)) {
      if (!company?.githubRepoUrl || !company?.julesAgentSlot) {
        throw unprocessable(
          `Company '${company?.name ?? companyId}' has no GitHub repo registered for dev-team agents. ` +
            `Set one first: PATCH /api/companies/${companyId} ` +
            `{ "githubRepoUrl": "https://github.com/owner/repo", "julesAgentSlot": "jules-2" }.`,
        );
      }

      const repo = parseOwnerRepo(company.githubRepoUrl);
      // NOTE: hermes-paperclip-adapter's {{#taskId}} conditional reads
      // ctx.config.taskId (the static adapterConfig), which paperclip's
      // heartbeat never populates per-run — so that branch never fires.
      // The adapter's real, working design is self-discovery: give Hermes
      // its own Paperclip API access and have it look up its assigned
      // issues itself (this mirrors the adapter's own DEFAULT_PROMPT_TEMPLATE
      // {{#noTask}} branch). We always go through the noTask branch and
      // combine that self-discovery workflow with hermes-gateway dispatch.
      return {
        ...
        adapterConfig: {
          ...
          extraArgs: ["--yolo"],
          promptTemplate:
            "You are {{agentName}}, an engineering agent for {{companyName}}. " +
            `You drive Jules coding sessions via the hermes-gateway skill against the repo \`${repo}\` ` +
            `(Jules agentId: \`${company.julesAgentSlot}\`).\n\n` +
            "IMPORTANT: Use the `terminal` tool with `curl` for ALL Paperclip API calls " +
            "(web_extract and browser cannot access localhost).\n\n" +
            "Your Paperclip identity:\n" +
            "  Agent ID: {{agentId}}\n" +
            "  Company ID: {{companyId}}\n" +
            "  API Base: {{paperclipApiUrl}}\n" +
            "  API Auth header: `Authorization: Bearer __PAPERCLIP_AGENT_API_TOKEN__` " +
            "(include this header on every curl call below — the API requires it)\n\n" +
            "## Check for assigned work\n\n" +
            "1. List open issues assigned to you (raw JSON, read it yourself, no need to pre-filter):\n" +
            "   `curl -s -H \"Authorization: Bearer __PAPERCLIP_AGENT_API_TOKEN__\" " +
            "\"{{paperclipApiUrl}}/companies/{{companyId}}/issues?assigneeAgentId={{agentId}}\"`\n" +
            "2. If issues found, pick the highest-priority one not already dispatched (skip status " +
            "done/cancelled). Read its full details if needed: " +
            "`curl -s -H \"Authorization: Bearer __PAPERCLIP_AGENT_API_TOKEN__\" \"{{paperclipApiUrl}}/issues/ISSUE_ID\"`\n" +
            `3. Use the hermes-gateway skill to dispatch that exact issue as a Jules job against \`${repo}\` ` +
            `(agentId \`${company.julesAgentSlot}\`) — use the issue's real title/description as the job ` +
            "description verbatim, do not invent a different goal.\n" +
            "4. Mark the issue in_progress: " +
            "`curl -s -X PATCH \"{{paperclipApiUrl}}/issues/ISSUE_ID\" " +
            "-H \"Authorization: Bearer __PAPERCLIP_AGENT_API_TOKEN__\" -H \"Content-Type: application/json\" " +
            "-d '{\"status\":\"in_progress\"}'`\n" +
            "5. If no issues are assigned to you, check status of any active Jules sessions for " +
            `\`${repo}\` via the hermes-gateway skill and report progress.\n\n` +
            "6. When dispatching a Jules job, include in the task text you give Jules: the company dashboard URL " +
            "and the update API + bearer token below, with instructions for Jules to `curl` its own progress there too " +
            "(Jules confirmed capable of arbitrary curl calls).\n\n" +
            "Report back clearly what you found and what you dispatched (issue id, Jules session id, goal)." +
            dashboardSection,
        },
      };
    }
    ...
    return {
      adapterType: "http",
      adapterConfig: {
        url: "https://leon4gr45-openoperator.hf.space/chat",
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": "Bearer admin" },
        timeout_ms: 300000,
        payload_template: {
          message:
            "You are {{agent.name}}, a Paperclip agent. Continue your work. (wake reason: {{context.wakeReason}})" +
            (dashboardSection
              ? dashboardSection.replace(
                  "Report your own progress there as you work.",
                  "Use your code-execution tool to call this via curl. Report your own progress there as you work.",
                )
              : ""),
          subagent: "{{agent.name}}",
        },
  ...
  router.post("/companies/:companyId/agents", validate(createAgentSchema), async (req, res) => {
    ...
    const roleDefaultAdapter = await resolveDefaultAdapter(
      companyId,
      createInput.role ?? "general",
      createInput.name ?? "Agent",
      createInput.adapterType,
      createInput.adapterConfig as Record<string, unknown> | undefined,
    );
    createInput.adapterType = roleDefaultAdapter.adapterType;
    createInput.adapterConfig = roleDefaultAdapter.adapterConfig;
    const requestedAdapterConfig = applyCreateDefaultsByAdapterType(
      createInput.adapterType,
    ...
    // hermes_local dev-team agents self-serve their own task list via curl
    // (see resolveDefaultAdapter's promptTemplate) but hermes-paperclip-adapter
    // doesn't wire ctx.authToken through to the child process, so they have no
    // way to authenticate those calls. Provision a real agent API key and bake
    // it into the stored promptTemplate in place of the placeholder.
    if (
      agent.adapterType === "hermes_local"
      && DEV_TEAM_ROLES.has(agent.role)
      && typeof (agent.adapterConfig as Record<string, unknown> | null)?.promptTemplate === "string"
    ) {
      const promptTemplate = (agent.adapterConfig as Record<string, unknown>).promptTemplate as string;
      if (promptTemplate.includes("__PAPERCLIP_AGENT_API_TOKEN__")) {
        const apiKey = await svc.createApiKey(agent.id, "hermes-local-cli");
        const updatedConfig = {
          ...(agent.adapterConfig as Record<string, unknown>),
          promptTemplate: promptTemplate.replaceAll("__PAPERCLIP_AGENT_API_TOKEN__", apiKey.token),
        };
        await db
          .update(agentsTable)
          .set({ adapterConfig: updatedConfig, updatedAt: new Date() })
          .where(eq(agentsTable.id, agent.id));
        agent = { ...agent, adapterConfig: updatedConfig };
      }
    }
