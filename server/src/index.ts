...
import { firmSyncService, heartbeatService, reconcilePersistedRuntimeServicesOnStartup, routineService } from "./services/index.js";
...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.heartbeatSchedulerEnabled) {
    ...
    const firmSyncEnabled = process.env.PAPERCLIP_FIRM_SYNC_ENABLED !== "false";
    const firmSync = firmSyncEnabled ? firmSyncService(db as any) : null;
    ...
    setInterval(() => {
      ...
      // Refresh per-company Firm snapshots that have aged past a day. The tick
      // runs frequently but only re-renders stale companies, so it self-throttles
      // to roughly once per company per day.
      if (firmSync) {
        void firmSync
          .tickDailySync(new Date())
          .then((result) => {
            if (result.synced > 0) {
              logger.info({ ...result }, "firm daily sync refreshed company snapshots");
            }
          })
          .catch((err) => {
            logger.error({ err }, "firm daily sync tick failed");
          });
      }
