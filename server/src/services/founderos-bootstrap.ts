...
export function founderOsBootstrapService(db: Db, options: FounderOsBootstrapOptions) {
  async function bootstrap(companyId: string, repo: CompanyRepositoryInspection) {
    ...
    return db.transaction(async (tx) => {
      ...
      const [manager] = await tx.insert(agents).values({ companyId, name: "Founder Manager", role: "ceo", title: "FounderOS Founder Manager", adapterType: "hermes_local", adapterConfig: { model: process.env.FOUNDER_MANAGER_MODEL ?? "alias-fast", timeoutSec: Number(process.env.FOUNDER_MANAGER_TIMEOUT_SEC ?? 900) }, status: "idle", capabilities: "result judgment, governance, prioritization", metadata: { founderosTemplateId: "founder-manager", contextVersion: FOUNDEROS_CONTEXT_VERSION } }).returning();
