...
```text
                  ...
                  │                       │
                  ▼                       ▼
           Hermes founder manager          Jules fleet
             + the configured Hermes model        profile 1..4..N
                  │                       │
                  │                       ▼
                  │                company GitHub repo
                  │                       │
...
Hermes Agent = managerial cognition / judging / triage
...
the configured Hermes model may classify/advise, but hard quota accounting and fairness rules must remain deterministic.
...
## 12. Hermes Agent manager

Paperclip already has an `hermes_local` adapter. Keep that capability, but the FounderOS managerial role should be conceptually separate from the company execution plane.

Hermes Agent should run beside Paperclip in the Hugging Face Space and use a model/provider selected through its normal environment-driven configuration.
...
Hugging Face Space
├── Paperclip server
└── Hermes founder manager service
    └── the configured Hermes model
```

Hermes Agent can be installed into the Space image and run as a persistent loopback-only headless service rather than cold-starting for every manager decision.
...
```text
HERMES_MODEL=<provider/model>
OPENROUTER_API_KEY=<secret>  # or the selected provider key
...
## 13. Jules session specification generated by Hermes Agent

Hermes Agent should render every Jules assignment from a single structured session template.
...
managerNotes: |
  {{OPTIONAL_HERMES_MANAGER_CONTEXT}}
```
...
## 13.1 Goal support and versioned resource packs

Skills define reasoning behavior; they do not carry reusable source material. Goals attach the exact support required for an outcome through `requiredSkills`, `supportPacks`, `requiredCapabilities`, `inputPaths`, `outputPaths`, `acceptanceCriteria`, and `cannotCompleteIf`.

Resource pack content lives in the imported company repository under `.founderos/support/`. Paperclip stores only a versioned snapshot manifest containing the pack ID/version/tier, repo-local install path, source repository and immutable source commit, file inventory, source guidance, and quality gates. Required packs must resolve before Jules proceeds; optional packs may be absent.

The Goal Support Resolver produces the narrow set of skills, resources, tools, artifact paths, and gates for the current goal. The Jules prompt references those repo-local paths rather than embedding the full resource library. This preserves reproducibility without flooding every session with irrelevant methodology.

Support tiers are:

```text
core          always installed company-wide
domain        installed when the company enters a workflow
goal_specific attached only to an applicable goal/outcome
```

Resource content remains owned by the FounderOS company-package repository. Paperclip owns resolution, version provenance, company scoping, and prompt delivery.
...
## 24. Paperclip / Hermes Agent / Jules boundaries
...
### Hermes Agent owns reasoning about
...
Always run deterministic checks before asking the configured Hermes model to judge quality.
...
Hermes Agent secrets and Jules profile credentials must be stored as environment secrets, never committed.
...
run pending deterministic validations
  ↓
invoke the configured Hermes model judge where needed
  ↓
resume queue
```

Do not assume local Hermes Agent conversational state is durable. Treat Hermes Agent memory as a cache; Paperclip DB + GitHub + Jules remote state are authoritative.
...
The existing `hermes_local` adapter can remain available, but the new FounderOS execution adapter is `jules`, not `hermes_local`.

The Hermes founder manager is a supervisory component, not the company execution runtime.
...
## 32. Hermes founder manager implementation shape
...
call the configured Hermes model through Hermes Agent or an OpenAI-compatible route
...
Never require company truth to exist only in a transient Hermes Agent session, Jules activity stream, or Linear issue.
...
This contract should be what Hermes Agent uses to render the Jules session prompt.
...
### Phase 5 — Hermes founder manager

- Configure the Hermes model and provider using environment secrets.
...
company/repo/source binding
   ↓
Hermes Agent chooses/bundles valuable outcome
   ↓
...
deterministic validators
   ↓
the configured Hermes model judge
   ↓
...
> Paperclip controls. Hermes Agent reasons. Jules works. Linear plans. Firm structures. GitHub remembers. Evidence decides.
