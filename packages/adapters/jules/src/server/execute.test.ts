...
describe("Jules REST adapter", () => {
  it("creates a remote session without using a local cwd and never self-accepts completion", async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({ id: "remote-1", state: "COMPLETED", pullRequestUrl: "https://github.com/acme/co/pull/1" }), { status: 200 }));
  ...
  it("resumes the same remote session instead of creating a duplicate", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ id: "remote-1", state: "IN_PROGRESS" }), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
