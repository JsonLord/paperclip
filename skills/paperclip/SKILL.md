...
**Common mistake:** The `heartbeat run` command uses `--agent-id`, NOT `--company-id`. The `--company-id` option is used with other commands like `issue create`, `issue update`, `company dashboard`, etc. Never use `--company-id` with `heartbeat run`.
...
**Important:** The `heartbeat run` command requires `--agent-id`, NOT `--company-id`. The `--company-id` option is only used with other commands like `issue create`, `issue update`, etc. Do NOT use `--company-id` with `heartbeat run`.
